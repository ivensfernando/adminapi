...
func OrderControllerHydra(
	...
	// 1. Create client
	c, err := connectors.NewGooeyClient("https://trade.gooeytrade.com")
	if err != nil {
		_ = orderRepo.UpdateStatusWithAutoLog(
			ctx,
			newOrder.ID,
			model.OrderExecutionStatusError,
			"hydra - NewGooeyClient failed",
		)
		return fmt.Errorf("hydra - NewGooeyClient failed: %v", err)
	}

	// 2. Login
	if err := c.Login(ctx, "fptech", config.HydraUsername, config.HydraPassword); err != nil {
		_ = orderRepo.UpdateStatusWithAutoLog(
			ctx,
			newOrder.ID,
			model.OrderExecutionStatusError,
	...
	resp, status, err := c.PlaceMarketOrder(
		...
		connectors.WithRequestID("gwt-uid-1115-3e0f0dcb-719f-4830-802c-7123190bd8a6"),
	)
	if err != nil {
		_ = orderRepo.UpdateStatusWithAutoLog(
			ctx,
